...
Full-image averages can hide a small but severely overbright or clipped lamp, sign, window, reflection, or cloud. Divide portrait images into a fixed `16 × 12` grid and landscape images into `12 × 16`, then measure near-white luma 235–255, bright luma 250–255, white clipping at luma 254–255, and any-channel clipping within every tile. Record the peak values, triggered-tile count, and the highest-risk tile's pixel and normalized bounds in `local_highlight_check`. Require review when any tile contains at least `20%` near-white pixels, at least `15%` bright highlights, or at least `5%` white clipping. The any-channel value is diagnostic only because a naturally saturated color may clip one channel without looking overexposed.

Set `quality_check.status` to `review_required` when black clipping is at least 1%, or when deep shadows cover at least 20% while median luma is at most 40, or when white clipping is at least 1%, or when bright highlights cover at least 5%, or when the local highlight check triggers. Also require review when the final JPEG median remains at least `0.08` below the preview median and is no more than 75% of that reference in a scene whose preview median is at least `0.25`. Record the final-to-preview median gap and ratio in `scene_midtone_reference`. These thresholds are conservative review triggers, not automatic exposure verdicts. Always inspect the reported peak tile at full resolution; legitimate night scenes, silhouettes, snow, the sun, and specular reflections may cross the thresholds.
